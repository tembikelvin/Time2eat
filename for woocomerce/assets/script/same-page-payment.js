...
function tzPushWooError(message){
  ...
  if(container && container.length){
    container[0].innerHTML = `<div class="wc-block-components-notice-banner is-error" role="alert" data-id="billing_last_name">
...
async function toggleCheckoutButton(){
  const tzPayments = [];
  if(window.orangeMoney){
    tzPayments.push(orangeMoney.id)
  }
  if(window.mtnMoMo){
    tzPayments.push(mtnMoMo.id)
  }
  const formData = new FormData(tzForm);
  if(tzPayments && tzPayments.indexOf(formData.get("payment_method")) < 0){
...
const tzTriggerPayment = async function (source = 1){
  ...
  try{
      ...
      form.set('tz_nonce', tzGlobal.nonce);
...
function tzGetRefreshTemplate (){
  ...
  <div class="tz-overlay-container">
    <div class="tz-overlay-content">
      ...
      <div class="tz-text-center tz-mt-20"></div>
    </div>
  </div>
`;
}
 ...
 * @returns template
 */
function tzGetAuthTemplate(amount = 0, currency = 'XAF', showCancelButton = false){
  return `
    <div class="tz-overlay-container">
      <div class="tz-overlay-content">
        <div class="tz-row">
        ...
        <div class="tz-mt-50 tz-text-center cancel-tz-transaction" id="cancel-tz-transaction">
          <div class="tz-w-100" style="display: ${showCancelButton? 'block': 'none'}">
            <button class="tz-danger-btn" onclick="cancelTransaction()">Cancel transaction</button>
          </div>
        </div>
        <div class="tz-text-center tz-mt-20"></div>
      </div>
...
/**
 * This will trigger pulling every 2 seconds and will expire after 1 / 2 minutes depending on the value of @intervalCounter if user doesn't authorize current transaction
 */
function tzStartPulling() {
  intervalCounter = 60;
  interval = setInterval(tzGetTransactionStatus, 2000)
}

/**
 * This will stop pulling the backend if transaction was successful or pulling timeout reached
 */
function tzStopPulling() {
  tzHideOverlay();
 ...
 */
function tzShowAuthTemplate(amount, currency, showCancelButton = false) {
  overlay.innerHTML = tzGetAuthTemplate(amount, currency, showCancelButton);
  // overlay.style.display = 'block';
}
...
async function tzRefreshTransactionStatus(){
  ...
  const formData = {
    ...
    nonce: tzGlobal.nonce
  };

  tzShowLoader();
  
  await jQuery.post(`${tzGlobal.paymentVerificationUrl}`,formData, function(res) {
    if(res && res.success){
      ...
      if(transaction.status == "SUCCESSFUL"){
        tzClearCart();
        tzShowRedirection();
        location.href = `${transaction.links.returnUrl}?transactionId=${transaction.transactionId}&requestId=${transaction.requestId}`;
        return;
      }
      ...
      tzShowRefreshTemplate(transaction.amount + (transaction.fee || 0), transaction.currencyCode );
    }else{
      tzHideOverlay();
      tzStopPulling();
...
/**
 * This is the method called during pulling and it checks for transaction status for the created payment request
 */
async function cancelTransaction(){
  const formData = {
    ...
    await jQuery.post(`${tzGlobal.cancelPaymentUrl}`,formData, function(res) {
      if(res && res.success){
        transaction = res.data;
        tzShowAuthTemplate(transaction.amount + (transaction.fee || 0), transaction.currencyCode );
        return processTransaction(transaction);
      }else{
        tzHideOverlay();
        tzStopPulling();
  ...
  const formData = {
    ...
    await jQuery.post(`${tzGlobal.paymentVerificationUrl}`,formData, function(res) {
      ...
      if(failedAttempts >=15){
        tzHideOverlay();
        tzStopPulling();
        tzPushWooError(res.errorMsg || 'Failed to process transaction');
      }
      ...
      if(res && res.success){
        failedAttempts = 0;
        transaction = res.data;
        tzShowAuthTemplate(transaction.amount + (transaction.fee || 0), transaction.currencyCode, intervalCounter <= 52 );
        return processTransaction(transaction);
      }else{
        failedAttempts+=1;
      }
