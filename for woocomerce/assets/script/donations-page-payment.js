...
async function toggleCheckoutButton(){
  const formData = new FormData(tzForm);
  if(window.tzPayments && window.tzPayments.indexOf(formData.get("payment_method")) < 0){
...
const tzTriggerDonationPayment = async ()=>{
  ...
  tzShowLoader();
    await jQuery.post(`${tzGlobal.donationUrl}`,formData, function(res) {
      if(res && res.success){
        ...
        if(transaction && transaction.links && transaction.links.paymentAuthUrl){
          ...
          // show error
          errorMessage = transaction.errorMsg || transaction.errorMessage || 'Failed to create transaction';
        }
      }else if(res.errorMsg || res.errorMessage){
        // show error
        errorMessage = res.errorMsg || res.errorMessage || 'Failed to create transaction';
      }
      tzInitDonation();
    }).fail(e=>{
      // show error
