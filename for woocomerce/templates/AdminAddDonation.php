<?php
  ...
  if(count($_POST) && ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'tzpg_auth_nonce'))){
    die();
  }
  ...
  $title = sanitize_text_field($_POST['title'] ?? $title);
  $successMessage = sanitize_text_field($_POST['success_message'] ?? $successMessage);
  $amount = sanitize_text_field($_POST['amount'] ?? $amount);
  $target = sanitize_text_field($_POST['target'] ?? $target);
  $donateText = sanitize_text_field($_POST['button_text'] ?? $donateText);
  $currency = sanitize_text_field($_POST['currency'] ?? $currency);
  $backgroundColor = sanitize_text_field($_POST['background_color'] ?? $backgroundColor);

  $shortCodes = '';

  $donationNotFound = false;



  if(isset($_GET['id'])){
    $id = (int) sanitize_text_field($_GET['id']) ?? null;
    if($id){
      $pageTitle = "Update Donation";
      $donation = $donations->getDonation($id);
      if(!$donation){
        $donationNotFound = true;
      }else{
      ...
      <div style=\"font-weight: bold\">
        <br />
        <p>Short code: <code> [tz_pg_donations id=". esc_html($id). "] </code><div style=\"font-weight: 400\">Insert this short code where you want your donation / request button to be displayed</div></p>
        <br />
        <p>Progress bar short code: <code> [tz_pg_progress id=". esc_html($id). "] </code><div style=\"font-weight: 400\">Insert this short code where to display a progress bar (if you entered a target amount)</div></p>
        <br />
        </div>
      ";
    }
  ...
  if($redirectScript){
    wp_redirect($redirectScript);
    die();
  }

  
  echo '<div class="wrap"><h1 class="wp-heading-inline">', esc_html($pageTitle), '</h1>
  <a href="'.admin_url( '/admin.php?page=tranzak_payment_gateway_donations_add').'" class="page-title-action">Add new</a><br /><br /><hr />
  ';
  echo '<form method="post">';

  foreach($errors as $error){
    echo '<p style="background: #f002; color: #c00; padding: 8px; border-radius: 4px">', esc_html($error), '</p>';
  }

  if($pageSuccessMessage){
    echo '<p style="background: #0f01; color: #0a0; padding: 8px; border-radius: 4px">', esc_html($pageSuccessMessage), '</p>';
  }
  ...
  echo '
    ...
    if($field['type'] == 'input'){
      echo '
      <input maxlength="255" type="', esc_attr($field['input_type']),'" class="', esc_attr($field['size']), '" id="', esc_attr($field['id']), '" name="', esc_attr($field['name']), '" ', ($field['required'] == 'true'? 'required': '').' value="', esc_attr($field['value']),'"/>
      ';
    }
    else if($field['type'] == 'textarea'){
      echo '
      <textarea type="', esc_attr($field['input_type']), '" class="regular-text" rows="4" id="', esc_attr($field['id']), '" name="', esc_attr($field['name']), '" '.($field['required'] == 'true'? 'required': '').'>', esc_textarea($field['value']), '</textarea>
      ';
    }
    else if($field['type'] == 'select'){
      echo '<select name="', esc_attr($field['name']), '" id="', esc_attr($field['id']), '" class="regular-text" value="', esc_attr($field['value']),'">';

      for($i=0; $i<count($field['options']); $i++){
        echo '<option value="', esc_attr($field['options'][$i]),'"'. ($field['options'][$i] == $field['value']? 'selected': ''), '>', esc_html($field['options'][$i]), '</option>';
      }
    echo '</select>';
      // echo '
    ...
    if($field['required'] == 'true'){
      echo '
      <br />
      <label style="color: #b00;" for="', esc_attr($field['id']), '">Required</label>
      ';
    }
