...
namespace Tranzak_PG\Api\Callbacks;
...
class AdminCallbacks extends BaseController{
  ...
  public function appIdField($args){
    ...
    echo '<input type="text" name="', esc_attr($optionName), '[app_id]" id="tz_app_id" class="regular-text" required value="', esc_attr($appId).'" placeholder="Enter App ID for this app (required)" />
  ...
  public function apiKey($args){
    ...
    echo '<input type="text" name="', esc_attr($optionName), '[api_key]" id="tz_api_key" class="regular-text" required value="', esc_attr($apiKey) . '" placeholder="Enter API Key for this app (required)" />
  ...
  public function authKey($args){
    ...
    echo '<input type="text" name="', esc_attr($optionName), '[auth_key]" id="tz_auth_key" class="regular-text" required value="', esc_attr($authKey), '" placeholder="Enter Auth Key for this app (required)" />
    </br><label for="tz_api_key" class="tz-admin-danger">Required</label><br>
    <p>
  ...
  public function orgIdField($args){
    ...
    echo '<input type="text" name="', esc_attr($optionName), '[org_id]" id="tz_org_id" class="regular-text" value="', esc_attr($orgId).'" placeholder="Enter Org ID (Tranzak ID of the owner of this app)" />
          <div class="tz-style-pt pt-1 p-1">
  ...
  public function currencyCodeField($args){
    ...
    // $orgId = esc_attr( get_option('tz_org_id'));
    echo '<select name="', esc_attr($optionName), '[currency]" id="tz_currency" class="regular-text" value="'.$currency.'">';

    for($i=0; $i<count($currencies); $i++){
      echo '<option value="', esc_attr($currencies[$i]),'"', ($currencies[$i] == $currency? 'selected': '') .'>', esc_html($currencies[$i]),'</option>';
    }
    echo '</select>
          <div class="tz-style-pt pt-1 p-1">
  ...
  public function envField($args){
    ...
    // $orgId = esc_attr( get_option('tz_org_id'));
    echo '<select name="', esc_attr($optionName), '[env]" id="tz_env" class="regular-text" value="', esc_attr($env),'">';

    for($i=0; $i<count($environments); $i++){
      echo '<option value="', esc_attr($environments[$i]),'"', ($environments[$i] == $env? 'selected': '') .'>'.ucfirst(esc_html($environments[$i])).'</option>';
    }
    if($env !== 'production'){
      // echo '</select>
  ...
  public function returnUrl($args){
    ...
    echo '<input type="text" name="',  esc_attr($optionName), '[return_url]" id="tz_return_url" class="large-text" required value="', esc_attr($returnUrl), '" placeholder="URL to redirect to after a transaction is completed (required)" />
    <br />
    <p>
      All transactions will redirect to this page for verification.  If you wish to create your own dedicated page for this, create a page, replace the return URL above with the link to your page and add the following shortcode to your page
      <code>[', esc_html($this->transactionShortCode),']</code>
    </p>
