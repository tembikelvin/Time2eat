...
class Donations extends \WP_List_Table
{
  ...
  private function get_table_data($search = '')
  {
    ...
    $table = $wpdb->prefix . sanitize_text_field($this->baseController->wpDonationsTable);

    if (!empty($search)) {
      return $wpdb->get_results(
        $wpdb->prepare("SELECT * from {$table} WHERE title Like %s OR description Like %s", ['%'. $wpdb->esc_like($search).'%', '%'. $wpdb->esc_like($search).'%']),
        ARRAY_A
      );
    } else {
      return $wpdb->get_results(
  ...
  function get_columns()
  {
    $columns = array(
      ...
      'short_code' => __('Short code', 'tranzak-payment-gateway'),
      'amount' => __('Amount', 'tranzak-payment-gateway'),
      'success_message' => __('Success message', 'tranzak-payment-gateway'),
      'progress_short_code' => __('Progress bar short code', 'tranzak-payment-gateway'),
      'button_text' => __('Button text', 'tranzak-payment-gateway'),
  ...
  function usort_reorder($a, $b)
  {
    ...
    $order = (!empty($_GET['id'])) ? sanitize_text_field($_GET['id']) : 'desc';
  ...
  {
    $actions = array(
      'edit' => sprintf('<a href="'.admin_url( '/admin.php?page=tranzak_payment_gateway_donations_add&id='.$item['ID'].'">' . __('Edit', 'tranzak-payment-gateway') . '</a>', $_REQUEST['page'], 'edit', $item['ID'])),
      'delete' => sprintf('<a href="?page=%s&action=%s&element=%s">' . __('Delete', 'tranzak-payment-gateway') . '</a>', $_REQUEST['page'], 'delete', $item['ID']),
    );
