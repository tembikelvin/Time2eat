...
class Transactions extends BaseController{
  ...
  public function createTransaction($amount, $currency, $description, $origin = 1, $reference = '', $url = '' ){
    ...
    global $wpdb;
    $table = $wpdb->prefix.sanitize_text_field($this->wpTransactionsTable);
  ...
  public function updateTransaction($transactionId, $status = 2, $externalTxnId = '', $amount = '', $currency = ''){
    ...
    $transaction = $wpdb->update($wpdb->prefix.sanitize_text_field($this->wpTransactionsTable), array('external_transaction_id' => $externalTxnId,  'status' => $status), $filter);
    if($transaction){
  ...
  public function getTransaction($transactionId){
    global $wpdb;
    $table = $wpdb->prefix.sanitize_text_field($this->wpTransactionsTable);
    $result = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE ID = %s", $transactionId));
    if($result){
      return $result;
    }
  ...
  public function getTransactionsByOrigin($origin, $reference, $status){
    global $wpdb;
    $table = $wpdb->prefix.sanitize_text_field($this->wpTransactionsTable);
    $sql = "SELECT * FROM $table WHERE origin = %s AND reference = %s";
    if(!empty($status)){
      $sql = "SELECT * FROM $wpdb->prefix$this->wpTransactionsTable WHERE origin = %s AND reference = %s AND status = %s";
    }
    $result = $wpdb->get_results($sql);
    $result = $wpdb->get_row($wpdb->prepare($sql, $origin, $reference, $status));
    if($result){
      return $result;
    }
    return false;
  ...
  public function cancelMapiRequest($requestId){

    $request =  $this->makeRequest($this->cancelRequestUrl, array("requestId" => $requestId));
    if($request['success'] == true){
      return $request['data'];
    }
    return false;
  ...
  public function createMapiTransaction(float $amount, string $currency, string $description, string $reference = '', $url = '', $extraParams = array(), $setPaymentExpirationTime = true ){
    ...
    $data = array_merge([
      ...
      'customization' => [
        ...
        'logoUrl' => $this->websiteLogo && count($this->websiteLogo) >0? $this->websiteLogo[0]: null
      ],
