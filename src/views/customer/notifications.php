...
$title = $title ?? 'Notifications - Time2Eat';
                ...
                <div class="tw-flex tw-items-center tw-space-x-2">
                    <?php if ($stats['unread'] > 0): ?>
                        <button id="markAllRead" 
                                class="tw-px-3 tw-py-2 tw-text-xs tw-font-medium tw-text-blue-600 tw-bg-blue-50 tw-border tw-border-blue-200 tw-rounded-lg hover:tw-bg-blue-100 tw-transition-colors tw-shadow-sm hover:tw-shadow-md"
                                title="Mark All Read">
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4">
                <div class="tw-flex tw-items-center tw-space-x-3">
                    <div class="tw-w-10 tw-h-10 tw-bg-blue-100 tw-rounded-lg tw-flex tw-items-center tw-justify-center">
                        <i data-feather="bell" class="tw-h-5 tw-w-5 tw-text-blue-600"></i>
                    </div>
                    <div>
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4">
                <div class="tw-flex tw-items-center tw-space-x-3">
                    <div class="tw-w-10 tw-h-10 tw-bg-green-100 tw-rounded-lg tw-flex tw-items-center tw-justify-center">
                        <i data-feather="shopping-bag" class="tw-h-5 tw-w-5 tw-text-green-600"></i>
                    </div>
                    <div>
    ...
    <div class="tw-px-4 tw-pb-6">
        <?php if (!empty($notifications)): ?>
            <div class="tw-space-y-3" id="notifications-container">
                <?php foreach ($notifications as $notification): ?>
                    <div class="notification-item tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4 tw-transition-all tw-duration-200 hover:tw-shadow-md <?= $notification['read_at'] ? 'tw-opacity-75' : 'tw-border-l-4 tw-border-l-blue-500' ?>"
                                    ...
                                    <div class="tw-flex tw-items-center tw-space-x-2 tw-ml-4">
                                        <?php if (!$notification['read_at']): ?>
                                            <button onclick="markAsRead(<?= $notification['id'] ?>)" 
                                                    class="tw-p-1 tw-rounded-full tw-text-gray-400 hover:tw-text-blue-600 tw-transition-colors"
                                                    title="Mark as read">
                                ...
                                <?php if (!empty($notification['action_url']) && !empty($notification['action_text'])): ?>
                                    <div class="tw-mt-3">
                                        <a href="<?= e($notification['action_url']) ?>" 
                                           class="tw-inline-flex tw-items-center tw-px-3 tw-py-2 tw-text-xs tw-font-medium tw-text-blue-600 tw-bg-blue-50 tw-border tw-border-blue-200 tw-rounded-lg hover:tw-bg-blue-100 tw-transition-colors">
        ...
        <?php else: ?>
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-8 tw-text-center">
                ...
                <p class="tw-text-gray-500 tw-mb-4">You'll see order updates, promotions, and important alerts here.</p>
                <a href="<?= url('/browse') ?>" 
                   class="tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw-bg-blue-600 tw-text-white tw-rounded-lg tw-text-sm tw-font-medium hover:tw-bg-blue-700 tw-transition-colors">
