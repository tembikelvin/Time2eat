...
$title = $title ?? 'Notifications - Time2Eat Rider';
$notifications = $notifications ?? [];
$stats = $stats ?? ['total' => 0, 'unread' => 0, 'urgent_unread' => 0, 'delivery_updates' => 0, 'system_alerts' => 0];
                ...
                <div class="tw-flex tw-items-center tw-space-x-2">
                    <?php if ($stats['unread'] > 0): ?>
                        <button id="markAllRead" 
                                class="tw-px-3 tw-py-2 tw-text-xs tw-font-medium tw-text-orange-600 tw-bg-orange-50 tw-border tw-border-orange-200 tw-rounded-lg hover:tw-bg-orange-100 tw-transition-colors tw-shadow-sm hover:tw-shadow-md"
                                title="Mark All Read">
        ...
        <div class="tw-grid tw-grid-cols-2 tw-gap-3 tw-mb-6">
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4">
                <div class="tw-flex tw-items-center tw-space-x-3">
                    <div class="tw-w-10 tw-h-10 tw-bg-orange-100 tw-rounded-lg tw-flex tw-items-center tw-justify-center">
                        <i data-feather="bell" class="tw-h-5 tw-w-5 tw-text-orange-600"></i>
                    </div>
                    <div>
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4">
                <div class="tw-flex tw-items-center tw-space-x-3">
                    ...
                    <div>
                        <p class="tw-text-2xl tw-font-bold tw-text-gray-900"><?= number_format($stats['delivery_updates']) ?></p>
    ...
    <div class="tw-px-4 tw-pb-6">
        <?php if (!empty($notifications)): ?>
            <div class="tw-space-y-3" id="notifications-container">
                <?php foreach ($notifications as $notification): ?>
                    <div class="notification-item tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-4 tw-transition-all tw-duration-200 hover:tw-shadow-md <?= $notification['read_at'] ? 'tw-opacity-75' : 'tw-border-l-4 tw-border-l-orange-500' ?>"
                        ...
                        <div class="tw-flex tw-items-start tw-space-x-3">
                            ...
                            <div class="tw-flex-shrink-0">
                                ...
                                $iconClass = match($notification['type']) {
                                    'delivery_update' => 'tw-text-blue-600 tw-bg-blue-100',
                                    'system_alert' => 'tw-text-red-600 tw-bg-red-100',
                                    'promotion' => 'tw-text-purple-600 tw-bg-purple-100',
                                    'user_action' => 'tw-text-orange-600 tw-bg-orange-100',
                                    'earnings' => 'tw-text-green-600 tw-bg-green-100',
                                    ...
                                    <div class="tw-flex tw-items-center tw-space-x-2 tw-ml-4">
                                        <?php if (!$notification['read_at']): ?>
                                            <button onclick="markAsRead(<?= $notification['id'] ?>)" 
                                                    class="tw-p-1 tw-rounded-full tw-text-gray-400 hover:tw-text-orange-600 tw-transition-colors"
                                                    title="Mark as read">
                                ...
                                <?php if (!empty($notification['action_url']) && !empty($notification['action_text'])): ?>
                                    <div class="tw-mt-3">
                                        <a href="<?= e($notification['action_url']) ?>" 
                                           class="tw-inline-flex tw-items-center tw-px-3 tw-py-2 tw-text-xs tw-font-medium tw-text-orange-600 tw-bg-orange-50 tw-border tw-border-orange-200 tw-rounded-lg hover:tw-bg-orange-100 tw-transition-colors">
        ...
        <?php else: ?>
            <div class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-gray-200 tw-p-8 tw-text-center">
                ...
                <p class="tw-text-gray-500 tw-mb-4">You'll see delivery assignments, earnings updates, and important alerts here.</p>
                <a href="<?= url('/rider/deliveries') ?>" 
                   class="tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw-bg-orange-600 tw-text-white tw-rounded-lg tw-text-sm tw-font-medium hover:tw-bg-orange-700 tw-transition-colors">
